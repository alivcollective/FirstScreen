...
import type { KmsArticleInput, ArticleStatus } from "@/types/kms"
import Link from "next/link"

export default function NewArticlePage() {
  const router = useRouter()
  const [saving, setSaving] = useState(false)
  const [form, setForm] = useState<KmsArticleInput & { content?: any }>({
    title_th: "", excerpt_th: "", content: {}, status: "draft", tags: [], seo_keywords: []
  })

  async function save(status?: ArticleStatus) {
    setSaving(true)
    const res = await fetch("/api/kms/articles", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(status ? { ...form, status } : form)
    })
    setSaving(false)
    if (res.ok) {
      const d = await res.json()
      if (d.id) router.push(`/admin/articles/${d.id}`)
    }
  }

  return (
    <AdminLayout title="บทความใหม่">
      <div className="flex flex-col h-[calc(100vh-3.5rem)]">
        <div className="flex items-center justify-between gap-3 px-6 py-3 border-b border-slate-800 bg-slate-900 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => save()} disabled={saving} className="flex items-center gap-1.5 rounded-lg border border-slate-700 hover:border-slate-600 px-3 py-1.5 text-xs font-medium text-slate-300 transition-colors">
              {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />} บันทึกร่าง
            </button>
        ...
        <div className="flex-1 overflow-hidden flex">
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <input value={form.excerpt_th ?? ""} onChange={e => setForm(p => ({ ...p, excerpt_th: e.target.value }))}
              placeholder="บทสรุปสั้นๆ (excerpt)"
            ...
            <RichEditor value={form.content} onChange={json => setForm(p => ({ ...p, content: json }))} minHeight={500} />
          </div>
          <div className="w-72 shrink-0 border-l border-slate-800 overflow-y-auto p-4 space-y-4 bg-slate-900/50">
            <div>
              ...
              <input value={form.slug ?? ""} onChange={e => setForm(p => ({ ...p, slug: e.target.value }))}
                placeholder="url-slug" className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-slate-300 focus:outline-none focus:border-teal-500" />
            </div>
            <div>
              ...
              <input value={form.seo_title ?? ""} onChange={e => setForm(p => ({ ...p, seo_title: e.target.value }))}
                placeholder="SEO title" className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-slate-300 focus:outline-none focus:border-teal-500" />
            </div>
            <div>
              ...
              <textarea value={form.seo_description ?? ""} onChange={e => setForm(p => ({ ...p, seo_description: e.target.value }))}
                rows={3} placeholder="Meta description" className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-xs text-slate-300 focus:outline-none focus:border-teal-500 resize-none" />
            </div>
          </div>
        </div>
      </div>
